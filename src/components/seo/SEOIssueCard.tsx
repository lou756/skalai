...
export function SEOIssueCard({ issue }: SEOIssueCardProps) {
  ...
  const getFixTypeInfo = () => {
    switch (issue.fixType) {
      ...
      case "automated":
        return { icon: <Zap className="h-3 w-3" />, label: t('issue.automated'), color: "text-green-600" };
      case "semi-automated":
        return { icon: <Settings className="h-3 w-3" />, label: t('issue.semiAutomated'), color: "text-blue-600" };
      default:
        return null;
    }
  };
